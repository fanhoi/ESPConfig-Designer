...
1. Соберите фронтенд в директории `esp-config-designer-frontend/`:
   ```bash
   cd esp-config-designer-frontend && npm run build
   ```
2. Скопируйте содержимое папки `dist/*` в `esp-config-designer/web/` (с перезаписью):
   ```bash
   rm -rf ../esp-config-designer/web/* && cp -R dist/* ../esp-config-designer/web/
   ```
3. Пересоберите/перезапустите аддон в Home Assistant.

> [!IMPORTANT]
> **ПРИМЕЧАНИЕ ДЛЯ ИИ-АГЕНТОВ (AI AGENTS) И РАЗРАБОТЧИКОВ**:
> При любом обновлении версии (в `config.json` и `package.json`) или изменении исходного кода фронтенда, **обязательно** выполните сборку фронтенда и скопируйте файлы из `dist/*` в `esp-config-designer/web/`. Если этого не сделать, ваши изменения не попадут в Docker-образ аддона Home Assistant и приложение останется старой версии без изменений. Чтобы Home Assistant увидел обновление и пересобрал аддон, также необходимо поднять версию (например, с `1.3.0` до `1.3.1`) во всех местах.
